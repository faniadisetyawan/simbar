<div class="modal fade zoomIn" id="formModal" tabindex="-1" aria-labelledby="formModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content border-0">
      ...
      <form action="{{ route('pembukuan.penghapusan.store') }}" method="post" class="tablelist-form">
        @csrf
...
@push('scripts-modal-penghapusan')
<script>
  ...
  const openFormModal = ({ data, doc }) => {
    ...
    let url = !!data ? 
      `{{ url('pembukuan/penghapusan/barang/${data.id}') }}` 
      : 
      `{{ route('pembukuan.penghapusan.store') }}`;
